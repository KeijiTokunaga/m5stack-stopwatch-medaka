...
void loop() {
  ...
  if(M5.BtnA.wasHold()) { night=!night; }
  ...
  energy.update(now,moved || M5.BtnA.isPressed() || M5.BtnB.isPressed()
    || touch.isPressed() || touch.wasClicked() || world.foodCount()>0);
  ...
  if(now-lastFrame>=energy.frameInterval()) { lastFrame=now; render(); }
  if(Serial.available()) {
    char c=Serial.read();
    if(c=='?') Serial.printf("MEDAKA imu=%d frames=%lu tilt=%.3f activity=%.3f gyroZ=%.1f slosh=%.1f food=%d eaten=%u light=%.1f fixed=%d eco=%d brightness=%d heap=%u\n",imu,(unsigned long)frames,world.tilt,world.activity,world.rollRate,world.slosh,world.foodCount(),world.eaten,world.lightClock,world.lightFrozen,energy.eco,appliedBrightness,ESP.getFreeHeap());
  }
